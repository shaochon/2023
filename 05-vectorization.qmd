...
```{r}
murders_sub = murders[murders$population>10^7,]
...
```{r}
city_temps |> mutate(temperature = 5/9*(temperature-32))
```
...
S_values <- sapply(n_values, S_n)

plot(n_values,S_values)
abline(h=pi^2/6,col='red')
```
...
```{r}
selected_states <- c("New York", "California", "Texas")
murders[match(selected_states,murders$state), ]
```
