...
```{r}
murders |> filter(rate<1) |> arrange(desc(population)) %>% head(1)
```
...
NHANES |> mutate(race = Race3) |>
  mutate(race =if_else(is.na(Race1),Race1,Race3)) |>
  select(race) |> table()
```
...
dat |> group_by(Race, Gender) |>
  summarise(smoke = sum(SmokeNow=='Yes',na.rm = T), n = sum(!is.na(Smoke100))) |>
