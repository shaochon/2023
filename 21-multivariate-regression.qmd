...
avg <- Batting |> 
  ...
  group_by(playerID) |>
  summarise(pa = sum(AB + BB), 
         singles = sum(H - X2B - X3B - HR) / pa, 
         bb = sum(BB) / pa, .groups = 'drop') |>
  filter(pa >= 100) |>
  select(playerID, singles, bb)
<<<<<<< Updated upstream
avg
...
(@) Now let's repeat the above for the `Teams` dataset. We will fit a linear regression model to obtain the effects of BB and HR on Runs (at the team level) in 1971. Use the `tidy` function in the **broom** package to obtain the results in a data frame. 
...
bb_models <- Teams |>
  ...
  group_by(yearID) |>
  nest() |>
  summarise(coefs = map(data, make_lm_model),.groups = 'drop') |>
  unnest(coefs) 
