...
```{r}
A = matrix(c(1,1,1,1,2,3,-1,-1,3,-1,4,-2,2,2,-2,-2),4,4,byrow=FALSE)
...
```{r}
ggplot(data.frame(d = as.matrix(d)[2,], y = y)  |> subset(d != 0), aes(x = factor(y), y = d)) + 
  geom_boxplot()
```
...
```{r}
middle_pixel_value <- mnist$train$images[,14*14]
#  Compute the distance between each feature and the feature representing the middle pixel
my_dist <- apply(mnist$train$images, 2, function(x){
  sqrt(sum((middle_pixel_value - x)^2))
})
image(1:28, 1:28, matrix(my_dist,28,28), col=gray.colors(256))
```
