...
movie25 <- movielens |>
  ...
  summarize(n = n(), 
            ...
            .groups = "drop") |>
  mutate(years_since_release = as.integer(format(Sys.Date(), "%Y")) - release_year,
          ratings_per_year = n / years_since_release
  ) |>
...
```{r}
 movielens |>
  ...
  summarize(avg_rating = mean(ratings),.groups = 'drop') |>
...
(@) 4. The `movielens` dataset also includes a time stamp. This variable represents the time and data in which the rating was provided. The units are seconds since January 1, 1970. Create a new column `date` with the date. Hint: use the `as_datetime` function in the __lubridate__ package.
```{r}
library(lubridate)
movielens <- movielens |>
  mutate(date = as_datetime(movielens$timestamp, origin = "1970-01-01"))
```


(@) 5. Compute the average rating for each week and plot this average against day. Hint: use the `round_date` function before you `group_by`.
...
movielens |>
  ...
  ggplot(aes(genres, avg, ymin = avg - se, ymax = avg + se)) +
  geom_errorbar() +
  geom_point() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```

(@) 7. The plot shows strong evidence of a genre effect. If we define $g_{u,i}$ as the genre for user's $u$ rating of movie $i$, which of the following models is most appropriate:

answer: c
This model is a fixed-effects model that includes a separate effect for each genre. The term $x_{u,i}^k$ is an indicator for whether movie $i$ is of genre $k$, and $\beta_k$ represents the effect of genre $k$ on the rating. This model allows for different effects for each genre and is appropriate if the genre effect varies by genre.
...
(@) 8. What are the top schools based on the average score? Show just the ID, size, and the average score.
```{r}
schools |> top_n(10, score) |> arrange(desc(score)) |> select(id, size, score)
```

(@) 9. Compare the median school size to the median school size of the top 10 schools based on the score.
```{r}
median(schools$size)
median(schools |> top_n(10, score) |> arrange(desc(score)) |> pull(size))
```


(@) 10. According to this test, it appears small schools are better than large schools. Five out of the top 10 schools have 100 or fewer students. But how can this be? We constructed the simulation so that quality and size are independent. Repeat the exercise for the worst 10 schools.
```{r}
median(schools |> top_n(-10, score) |> arrange(desc(score)) |> pull(size))

schools |> top_n(-10, score) |> filter(size<=100)
```

(@) 11. The same is true for the worst schools! They are small as well. Plot the average score versus school size to see what's going on. Highlight the top 10 schools based on the _true_ quality. Use the log scale transform for the size.
```{r}
schools |> 
  mutate(sum = sapply(scores, mean)) |>
  ggplot(aes(size, score)) +
  geom_point() +
  geom_point(data = schools |> top_n(10, quality), color = "red") +
  scale_x_log10()
```

(@) 12. We can see that the standard error of the score has larger variability when the school is smaller. This is a basic statistical reality we learned in the probability and inference sections. In fact, note that 4 of the top 10 schools are in the top 10 schools based on the exam score.
...
(@) 13. Notice that this improves things a bit. The number of small schools that are not highly ranked is now 4.  Is there a better $\lambda$? Find the $\lambda$ that minimizes the RMSE = $1/100 \sum_{i=1}^{100} (\mbox{quality} - \mbox{estimate})^2$.
...
(@)14.  Rank the schools based on the average obtained with the best $\lambda$. Note that few small schools are incorrectly included in the top.

```{r, eval=FALSE}
lambda <- lambdas[which.min(rmses)]
schools <- schools |> 
  mutate(sum = sapply(scores, sum),
         score_reg = mu + (sum - size*mu)/(size + lambda))
schools |> arrange(desc(score_reg))
```


(@)15. A common mistake to make when using regularization is shrinking values towards 0 that are not centered around 0. For example, if we don't subtract the overall average before shrinking, we actually obtain a very similar result. Confirm this by re-running the code from exercise 6 but without removing the overall mean. 
```{r, eval=FALSE}
lambdas <- seq(0, 50, 0.1)
rmses <- sapply(lambdas, function(lambda){
  schools |> 
  mutate(sum = sapply(scores, sum)) |>
  mutate(score_reg = mu + (sum)/(size + lambda)) |>
  summarize(mean(score_reg - quality)^2)
})
plot(lambdas, rmses, type = "l")
```
