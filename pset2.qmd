...
counts <- counts |> 
  ...
  select(year,sex,date,agegroup,outcome,population)
```
...
weekly_counts <- counts |> 
  mutate(firstday = floor_date(date, "week", week_start = 3)) |>
  group_by(firstday, sex,agegroup) |>
  summarize(N = n(),
            outcome = sum(outcome), 
...
```{r}
weekly_counts |>
  mutate(year = year(firstday)) |>
  filter(year < 2017) |>
  group_by(firstday) |>
  summarize(outcome = sum(outcome), .groups = "drop") |>
  ggplot(aes(x = firstday, y = outcome)) +
  geom_boxplot()  +
  labs(x = "Week", y = "Deaths per week")
...
```{r}
weekly_counts |>
  filter(firstday < "2017-09-20") |>
  mutate(median = median(outcome)) |>
  summarize(pct_below = mean(outcome < median))
```
